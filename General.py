...
# --- Descripción centrada debajo del título global ---
st.markdown(
    """
    <div style="
        width: 100%;
        ...
        line-height: 1.45;
    ">
        Análisis del comportamiento aéreo local en Argentina, en base a datos de vuelos comerciales provistos por la Administración Nacional de Aviación Civil (ANAC).
    </div>
    """,
    unsafe_allow_html=True,
)
...
def _icons_dir(this_file: Path) -> Path:
    candidates = [this_file.parent / "assets" / "icons", this_file.parent.parent / "assets" / "icons"]
    for c in candidates:
        if c.exists():
            return c
...
# === Carga de datos (dataset UNIFICADO) ===
DATA_DIR = _find_data_dir(Path(__file__).resolve())

@st.cache_data(show_spinner=True)
def load_data_unificado(data_dir: Path):
    """
    Lee el dataset unificado (vuelos, NO movimientos).
    Espera columnas: Fecha, Fecha_Hora_Local_despegue, Origen, Destino,
                     Aerolinea, Aeronave, Pasajeros, Vuelo_ID, Fecha_Hora_Local_aterrizaje, Tiempo_Vuelo
    """
    CANDIDATES = [
        "vuelos_preliminar.csv",
    ]
    path = None
    ...
    df = pd.read_csv(path, low_memory=False, parse_dates=["Fecha", "Fecha_Hora_Local_despegue", "Fecha_Hora_Local_aterrizaje"])
    # normalizamos texto
    for c in ["Origen", "Destino", "Aerolinea", "Aeronave", "Ciudad_O", "Provincia_O", "Ciudad_D", "Provincia_D"]:
        if c in df.columns:
            df[c] = _std_text(df[c])
    ...
    if "Pasajeros" in df.columns:
        df["PAX"] = pd.to_numeric(df["Pasajeros"], errors="coerce")
    else:
        df["PAX"] = np.nan

    # Tiempo de vuelo (a minutos)
    if "Tiempo_Vuelo" in df.columns:
        try:
            t = pd.to_timedelta(df["Tiempo_Vuelo"])
            df["MINUTOS_VUELO"] = (t.dt.total_seconds() / 60.0).round().astype("Int64")
        except Exception:
...
# Aeropuertos = unión Origen ∪ Destino
all_airports = sorted(pd.Index(df_all["Origen"].dropna().unique()).union(df_all["Destino"].dropna().unique()).tolist())
airport_labels = [f"{c} — {code_to_name.get(c, c)}" for c in all_airports]
...
with st.sidebar.container():
    st.caption("Aeropuerto (origen o destino)")
...
# Aplicación de filtros (O/D)
df = df_all.copy()
df = _apply(df, "AÑO", sel_years, exclude=excl_years)
if sel_months:
    df = df[~df["Fecha"].dt.month.isin(sel_months)] if excl_month else df[df["Fecha"].dt.month.isin(sel_months)]
df = _apply(df, "AEROLINEA", sel_aerolinea, exclude=excl_airline)

if sel_airports:
    if excl_airport:
        df = df[~(df["Origen"].isin(sel_airports) | df["Destino"].isin(sel_airports))]
    else:
        df = df[(df["Origen"].isin(sel_airports) | df["Destino"].isin(sel_airports))]

# ================= Contenido principal =================

# ---------- KPIs (sobre VUELOS, no movimientos) ----------
st.markdown("""
<style>
...
# Total de vuelos únicos
total_vuelos = int(df["Vuelo_ID"].nunique()) if "Vuelo_ID" in df.columns else len(df)

# Aerolíneas activas
aerolineas_activas = int(df["AEROLINEA"].nunique())

# Aeropuertos activos (origen + destino)
aeropuertos_activos = int(pd.Index(df["Origen"].dropna().unique())
                            .union(df["Destino"].dropna().unique())
                            .nunique())

# --- Ruta más operada ---
if not df.empty:
    ruta_top = (df.groupby(["Origen", "Destino"])["Vuelo_ID"]
                  .nunique()
                  .reset_index()
...
if not df.empty:
    ...
    mes_top = (df.groupby("_MES_NUM")["Vuelo_ID"]
                 .nunique()
                 .reset_index()
                 .sort_values("Vuelo_ID", ascending=False)
                 .head(1))
...
# ---------- Gráfico 1: Vuelos por año ----------
ICONS_DIR = _icons_dir(Path(__file__).resolve())
ICON_PATH = ICONS_DIR / "plane.png"

by_year = (df.groupby("AÑO").agg(VUELOS=('Vuelo_ID', 'nunique') if 'Vuelo_ID' in df.columns else ('Vuelo_ID','size')))
by_year = by_year.reset_index().sort_values("VUELOS", ascending=True)

fig_year = px.bar(by_year, x="VUELOS", y="AÑO", orientation="h", title="✈️ Vuelos por año", text_auto=True)
...
# ---------- Gráfico 2: Vuelos por mes (multi-año) ----------
...
if not df.empty:
    ...
    by_month_cal["MES"] = pd.Categorical(
        by_month_cal["_MES_NUM"].map(mes_nombres),
    ...
    mes_top_nombre = mes_nombres[mes_top_num]

    # === Colorear barras: highlight en la top ===
    colors = [
    ...
    fig_month_tot.update_traces(
        ...
        cliponaxis=False,
        hovertemplate="🗓️ <b>%{x}</b><br>✈️ Vuelos: %{y:,}<br>% participación: %{customdata:.1%}<extra></extra>",
        customdata=by_month_cal["PCT"].to_numpy()
    )

    # === Highlight con coronita 👑 encima de la barra top ===
    fig_month_tot.add_annotation(
        x=mes_top_nombre,
        y=max_vuelos + (max_vuelos * 0.05),
        text="👑",
        showarrow=False,
        font=dict(size=32),
        xanchor="center"
...
# ---------- Donut por aerolínea ----------
TOP_N = 10
by_airline = (df.groupby("AEROLINEA")["Vuelo_ID"].nunique().reset_index(name="VUELOS").sort_values("VUELOS", ascending=False).head(TOP_N))
...
# ---------- Widget: Top conexiones Origen → Destino (usando dataset UNIFICADO) ----------
...
def _wrap_html(txt: str, width: int = 28) -> str:
    try: return "<br>".join(textwrap.wrap(str(txt), width=width))
    except Exception: return str(txt)

# Agregado rutas sobre VUELOS (unique Vuelo_ID)
if "Vuelo_ID" in df.columns:
    df_pax_unique = df.groupby("Vuelo_ID")["PAX"].max()  # pax por vuelo sin duplicar
    df_tmp = df[["Vuelo_ID", "Origen", "Destino"]].drop_duplicates()

    df_tmp = df_tmp.merge(df_pax_unique, on="Vuelo_ID", how="left")

    rutas = (df_tmp.groupby(["Origen", "Destino"])
    ...
    rutas = (df.groupby(["Origen", "Destino"])
...
if rutas.empty:
    ...
    X_ARROW_A    = 0.46
    X_ARROW_B    = 0.74
    DEST_PANEL_L = 0.80   # panel destino SIN línea punteada
    DEST_PANEL_R = 0.97
    X_DEST       = 0.88
    X_PLANE      = 0.60
    Y_PLANE_OFF  = 0.30

    # Panel destino (sin borde/dash)
    ...
    fig_routes.update_layout(margin=dict(l=40, r=40, t=60, b=28),
                             height=max(440, 67 * n),
                             title=" ")
    fig_routes = style_fig(fig_routes); fig_routes = _force_axes_colors(fig_routes); fig_routes = _big_titles(fig_routes, size=24)
