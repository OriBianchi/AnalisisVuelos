...
df = eda.copy()
if sel_years:     df = df[df["AÑO"].isin(sel_years)]
if sel_aerolinea: df = df[df["AEROLINEA"].isin(sel_aerolinea)]
if sel_tipo:      df = df[df["TIPO_MOV"].isin(sel_tipo)]
if sel_airports:  df = df[df["AEROPUERTO"].isin(sel_airports)]
...
if det_path.exists():
    ...
    fig_top = px.bar(top10.sort_values("VUELOS"), x="VUELOS", y="NOMBRE", orientation="h",
                     title="🧭 Aeropuertos más transitados (Top 10)", text_auto=True)
...
st.dataframe(df[["Fecha", "AEROPUERTO", "AEROLINEA", "TIPO_MOV", "PAX", "AÑO", "MES"]].head(500),
